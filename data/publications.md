E-MRL: Cross-view Aligned Evidence-driven Multimodal Reinforcement Learning for Reliable 3D Tumor Analysis
Sijing Li, **Zhongwei Qiu✉**, Zhuoya Wang, Boxiang Yun, Zhenyu Yi, Jianwei Xu, Wenqiao Zhang, Yingda Xia, Ling Zhang.
MICCAI 2026, (✉ Corresponding author)
[paper](https://arxiv.org/abs/2606.23888)
@asset: 2026_C_MICCAI_E-MRL
---
EndoVLM: An Endoscopy Vision-Language Pre-training Model via Anatomy-Guided Sparsity and Progressive Alignment
Zhenyu Yi, Jianwei Xu, Yue Hu, **Zhongwei Qiu**, Sijing Li, Liang Huang, Bin Lv, Ling Zhang, Yingda Xia.
MICCAI 2026
[paper](assets/papers/2026_C_MICCAI_EndoVLM/paper.pdf)
@asset: 2026_C_MICCAI_EndoVLM
---
Regulating Anatomy-Aware Rewards via Trajectory-Integral Feedback for Volumetric Computed Tomography Analysis
Tianwei Lin*, **Zhongwei Qiu***, Jie Cao, Jiang Liu, Wenjie Yan, Bo Zhang, Yu Zhong, Wenqiao Zhang, Yingda Xia, Ling Zhang.
ICML 2026, (* Equal contribution)
[paper](https://arxiv.org/abs/2605.20277)
@asset: 2026_C_ICML_TIF-GRPO
---
TumorChain: Interleaved Multimodal Chain-of-Thought Reasoning for Traceable Clinical Tumor Analysis
Sijing Li*, **Zhongwei Qiu***, Jiang Liu, Wenqiao Zhang, Tianwei Lin, Yihan Xie, et al.
ICLR 2026, (* Equal contribution)
[paper](https://arxiv.org/abs/2603.05867)
@asset: 2026_C_ICLR_TumorChain
---
OmniCT: Towards a Unified Slice-Volume LVLM for Comprehensive CT Analysis
Tianwei Lin*, **Zhongwei Qiu***, Wenqiao Zhang, Jiang Liu, Yihan Xie, et al.
ICLR 2026, (* Equal contribution)
[paper](https://arxiv.org/abs/2602.16110) [code](https://github.com/alibaba-damo-academy/OmniCT)
@asset: 2026_C_ICLR_OmniCT
---
AI-based large-scale screening of gastric cancer from noncontrast CT imaging
Can Hu, Yingda Xia, Zhilin Zheng, **Zhongwei Qiu**, et al.
Nature Medicine
[paper](https://doi.org/10.1038/s41591-025-03588-x)
@asset: 2025_J_NatMed_GastricCancer
---
Bridging Local Inductive Bias and Long-Range Dependencies with Pixel-Mamba for End-to-end Whole Slide Image Analysis
**Zhongwei Qiu**, Hanqing Chao, Tiancheng Lin, Wanxing Chang, Zijiang Yang, et al.
ICCV 2025
[paper](https://arxiv.org/abs/2412.16711)
@asset: 2025_C_ICCV_PixelMamba
---
From Histopathology Images to Cell Clouds: Learning Slide Representations with Hierarchical Cell Transformer
Zijiang Yang*, **Zhongwei Qiu***, Tiancheng Lin, Hanqing Chao, et al.
Preprint, (* Equal contribution)
@asset: 2024_P_Preprint_CellClouds
---
Knowledge graph information bottleneck enhanced molecular representation learning
Jiaxin Dai, Dongmei Fu, **Zhongwei Qiu**, Lingwei Ma, Dawei Zhang.
Neural Networks
[paper](https://doi.org/10.1016/j.neunet.2025.107560)
@asset: 2024_J_NeuralNetworks_KGBottleneck
---
Multi-scale contrastive adaptor learning for segmenting anything in underperformed scenes
Ke Zhou, **Zhongwei Qiu**, Dongmei Fu.
Neurocomputing
[paper](https://arxiv.org/abs/2408.05936)
@asset: 2024_J_Neurocomputing_MultiScaleSAM
---
MM-NeRF: Multimodal-Guided 3D Multi-Style Transfer of Neural Radiance Field
Zijiang Yang, **Zhongwei Qiu**, Chang Xu, Dongmei Fu.
TVCG
[paper](https://arxiv.org/abs/2309.13607)
@asset: 2024_J_TVCG_MMNeRF
---
Pedestrian-Centric 3D Pre-collision Pose and Shape Estimation from Dashcam Perspective
Meijun Wang, Yu Meng, **Zhongwei Qiu**, Chao Zheng, Yan Xu, Xiaorui Peng, Jiao Gao.
NeurIPS 2024
[paper](https://proceedings.neurips.cc/paper_files/paper/2024/hash/2c428bb07062012236519b589db63f34-Abstract-Conference.html)
@asset: 2024_C_NeurIPS_Pedestrian3D
---
IHCSurv: Effective Immunohistochemistry Priors for Cancer Survival Analysis in Gigapixel Multi-stain Whole Slide Images
Yejia Zhang, Hanqing Chao, **Zhongwei Qiu**, Wenbin Liu, et al.
MICCAI 2024
[paper](https://arxiv.org/abs/2409.03804)
@asset: 2024_C_MICCAI_IHCSurv
---
HAP: Structure-Aware Masked Image Modeling for Human-Centric Perception
Junkun Yuan, Xinyu Zhang, Hao Zhou, Jian Wang, **Zhongwei Qiu**, et al.
NeurIPS 2023
[paper](https://arxiv.org/abs/2310.20695)
@asset: 2023_C_NeurIPS_HAP
---
Stable Diffusion is Unstable
Chengbin Du, Yanxi Li, **Zhongwei Qiu**, Chang Xu.
NeurIPS 2023
[paper](https://arxiv.org/abs/2306.02583)
@asset: 2023_C_NeurIPS_StableDiffusion
---
PSVT: End-to-End Multi-person 3D Pose and Shape Estimation with Progressive Video Transformers
**Zhongwei Qiu**, Yang Qiansheng, Jian Wang, Haocheng Feng, Junyu Han, Errui Ding, Chang Xu, Dongmei Fu, Jingdong Wang.
CVPR 2023
[paper](https://arxiv.org/abs/2303.09187)
@asset: 2023_C_CVPR_PSVT
---
Weakly-supervised Pre-training for 3D Human Pose Estimation via Perspective Knowledge
**Zhongwei Qiu**, Kai Qiu, Jianlong Fu, Dongmei Fu.
Pattern Recognition
[paper](https://arxiv.org/abs/2211.11983)
@asset: 2023_J_PR_WeaklySup3DPose
---
Learning Degradation-robust Spatiotemporal Frequency-Transformer for Video Super-Resolution
**Zhongwei Qiu**, Huan Yang, Jianlong Fu, Daochang Liu, Chang Xu, Dongmei Fu.
TPAMI
[paper](https://arxiv.org/abs/2208.03012) [code](https://github.com/ericzw/FTVSR)
@asset: 2023_J_TPAMI_FTVSR
---
DMIS: Dynamic Mesh-based Importance Sampling for Training Physics-Informed Neural Networks
Zijiang Yang, **Zhongwei Qiu**, Dongmei Fu.
AAAI 2023
[paper](https://arxiv.org/abs/2211.13944)
@asset: 2023_C_AAAI_DMIS
---
Learning spatiotemporal frequency-transformer for compressed video super-resolution
**Zhongwei Qiu**, Huan Yang, Jianlong Fu, Dongmei Fu.
ECCV 2022
[paper](https://arxiv.org/abs/2208.03012)
@asset: 2022_C_ECCV_FTVSR
---
IVT: An End-to-End Instance-guided Video Transformer for 3D Pose Estimation
**Zhongwei Qiu**, Qiansheng Yang, Jian Wang, Dongmei Fu.
ACM MM 2022 (Oral)
[paper](https://arxiv.org/abs/2208.03431)
@asset: 2022_C_ACMMM_IVT
---
Dynamic Graph Reasoning for Multi-person 3D Pose Estimation
**Zhongwei Qiu**, Qiansheng Yang, Jian Wang, Dongmei Fu.
ACM MM 2022
[paper](https://arxiv.org/abs/2207.11341)
@asset: 2022_C_ACMMM_DGR
---
DGCN: Dynamic Graph Convolutional Network for Efficient Multi-person Pose Estimation
**Zhongwei Qiu**, Kai Qiu, Jianlong Fu, Dongmei Fu.
AAAI 2020
[paper](https://ojs.aaai.org/index.php/AAAI/article/view/6867)
@asset: 2020_C_AAAI_DGCN
---
Learning Recurrent Structure-guided Attention Network for Multi-person Pose Estimation
**Zhongwei Qiu**, Kai Qiu, Jianlong Fu, Dongmei Fu.
ICME 2019 (Oral)
[paper](https://ieeexplore.ieee.org/document/8784906)
@asset: 2019_C_ICME_RSAN
